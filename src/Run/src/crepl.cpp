...
int CREPL(void){
	...
	ZPrompt_CheckEnv(configpath);
	if (configpath){
		sprintf(configpath, "%s\\conf\\%s", configpath, CONFIGFILE);
	}
	// ReadLine(configpath, &testProgM); // import config.ini

	int s = 1;									   // 状态
	int lines = 0;								   // 行数
	char* cmd1 = (char*)malloc(MaxLenStatements); // 存储普通状态的字符串
	char* FSU = (char*)malloc(MaxCharSize);
	char aStatement[100];
	...
	for (unsigned int cyclecounter = 0;; cyclecounter++){
		...
		fgets(cmd1, sizeof(cmd1), stdin); // 获取输入的字符串
		fflush(stdin);

		/* 判断是程序特殊指令，子函数内只判断不执行 */
		bool judgRet1 = IsItSpecialCommand(cmd1);
		if (judgRet1 == true){
			s = 1; // 是命令，执行完了
		}
		else{
			int judgRet2 = determineType(cmd1); /*判断是否是预处理命令 */
			if (judgRet2 == 1){
				testProgM.addInc(cmd1);
				s = 1;
			}
			else if (judgRet2 == 2){
				testProgM.addDef(cmd1);
				s = 1;
			}
			else if (judgRet2 == 3){
				testProgM.addFunc(cmd1);
			}
			else if (judgRet2 == 4 || judgRet2 == 5){
				testProgM.addStructUnion(cmd1);
			}
			else if (judgRet2 == 6){
				testProgM.addMainStatement(cmd1);
				...
				if (s == -1){ // 花括号未闭合
					if (olds == -1 || olds == -2 || olds == -3){
						...
						strcpy(FSU, cmd1); // 开始定义函数
					}
				}
				else if (s == -2){ // 未写完的普通语句
					if (olds == -1 || olds == -2 || olds == -3){
						strcat(aStatement, cmd1);
					}
					else{
						// aStatement[0] = '\0';
						strcpy(aStatement, cmd1); // 开始写普通语句
					}
				}
...
bool IsItSpecialCommand(const char* str){
	if (isMatch(str, R"(^\s*\.(exit|quit)\s*$)")) {
		...
		ColorPuts(0x04, "Invalid command!", 1);
	}
