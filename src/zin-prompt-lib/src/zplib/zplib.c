...
int ZPrompt_CheckEnv(char* homePath){
	if(homePath == NULL){
		puts("ChecEnv: Please allocate memory first!");
		return -1;
	}
...
void ZPrompt_SubprojVersion(const char* subprojName, const char* subprojVersion, const char* compiledTime, const char* compiledDate){
	printf("%s/%s version %s (compiled %s, %s)\n", MAIN_PROJ_NAME, subprojName, subprojVersion, compiledTime, compiledDate);
	printf("Copyright (c) %s %s. All rights reserved. %s\n\n", MAIN_PROJ_YEARS, AUTHOR, AUTHOR_CONTACT);
}

void ZPrompt_ReadManual(const char* path){
	;
}


// HEX -> DEC
int ConvertHex(char* hexadecimalStr, long* pDecimal){
    char* endptr;
...
int PutStr(const char* Str){ // puts()自动换行，printf()不想使用，故写了这个函数
	for(int i=0;;++i){
		if(*(Str+i)!='\0')
			putchar(*(Str+i));
		else
...
// UTF-8 检测函数
int IsUTF8(const char* data, int len) {
    int i = 0;
    while (i < len) {
...
void LoadLoop(const char* text, int style){
    char* sym_braille[9] = {"⣷","⣯","⣟","⡿","⢿","⣻","⣽","⣾","⣿"};
    char* sym_ascii[4] = {"-","\\","|","/"};
    typedef struct {
        char** sym; // 字符数组指针
