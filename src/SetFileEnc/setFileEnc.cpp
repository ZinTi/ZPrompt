    ...
    if(argc>4){
        ...
        if(!strncmp(argv[1], "--to=", 5)){
            ...
            if(!strcmp(argv[1]+5, "gb2312")||!strcmp(argv[1]+5, "GB2312")||!strcmp(argv[1]+5, "gbk")||!strcmp(argv[1]+5, "GBK")||!strcmp(argv[1]+5, "ANSI")){
                ...
                cout << "This encoding format is not supported: " << argv[1]+5 << endl;
            ...
            if(argc==4){
                ...
                cout << "Conversion completed: " << argv[3] << endl;
                ChangeConsoleTextColor(COLOR_PROMPT_DEFAULT);
            }else{
                ...
                if(CopyFileA(argv[2], backupFileName, false)==0){
                    ChangeConsoleTextColor(COLOR_PROMPT_WARN);
                    cout << "Failed to back up the file: " << argv[2] << endl;
                    ChangeConsoleTextColor(COLOR_PROMPT_DEFAULT);
                    return 1;
                }
                ...
                outputFile.close();
                ChangeConsoleTextColor(COLOR_PROMPT_OK);
                cout << "Conversion completed: " << argv[2] << endl;
                ChangeConsoleTextColor(COLOR_PROMPT_DEFAULT);
            }
        ...
        cout << "The " << argv[1] << " file is " << (isUTF8 ? "UTF-8" : "GBK/GB2312") << " encoded." << endl;
        ChangeConsoleTextColor(COLOR_PROMPT_DEFAULT);
    }else{
        ShowVersion();
...
void ShowVersion(void){
	cout << "SetFileEncoding version 1.0 (compiled " << __TIME__ << ", " << __DATE__ << ")" << endl;
	cout << "Copyright (c) 2024 Zeng Lai. All rights reserved. vip201@126.com\n" << endl;
}

void ShowManual(char* progName){
    //cerr << "Usage: " << progName << " <input file> <output file> [u-to-g|g-to-u]" << endl;
    cout << "Usage:\n\t" << progName << " [ --to=gbk | --to=utf-8 ] <input file> [output file]" << endl;
    cout << "If multiple encodings such as GB2312 and UTF-8 are present in the text, only one will be recognized." << endl;
}
